...
const mainIndex = mainIndexTOC.split("\n").map((line) => line.match(/<a href="part_(?<id>.+)\/.+ <\/span>(?<name>.+)<\/span>/).groups) as { id: string; name: string; }[];

const mainTOC = await Promise.all(mainIndex.map(async ({ id: partId, name: partName }) => {
